...
from pseudo_labels_utils.Label_generation import FT3D_label_generation
...
def main():
    ...
    for epoch in range(init_epoch, args.epochs):
        ...
        if epoch == 200:
            torch.save(state, os.path.join(checkpoints_dir, "model_200.tar"))
...
def train_sceneflow(model, train_loader, optimizer, epoch):
    ...
    for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), smoothing=0.9):
        pos1, pos2, norm1, norm2, flow, path, valid_list  = data
        ...
        valid_list = valid_list.cuda()
        if torch.sum(valid_list) == valid_list.shape[0]:
        # Skipping current training iteration, when loading invalid samples

            pred_flows = model(pos1, pos2, None, None)
            optimizer.zero_grad()

            # Generating pseudo labels
            with torch.no_grad():
                pseudo_gt = FT3D_label_generation(pred_flows.detach().clone(), pos1, pos2, norm1, norm2)
                pseudo_gt = pseudo_gt.detach()

            # Computing loss
            diff_flow = pred_flows.permute(0, 2, 1) - pseudo_gt.permute(0, 2, 1)
            loss1 = 0.02 * torch.norm(diff_flow, dim=2).sum(dim=1).mean()

            # Computing cycle consistency loss
            if epoch > 200:
                warped_pc1 = pos1 + pred_flows.permute(0, 2, 1)
                warped_pc1_detach = warped_pc1.detach()
                reverse_flow_pred = model(warped_pc1_detach, pos1, None, None)

                pc1_hat = warped_pc1_detach + reverse_flow_pred.permute(0, 2, 1)
                diff_flow = pc1_hat - pos1
                loss2 = 0.02 * torch.norm(diff_flow, dim=2).sum(dim=1).mean()
...
def eval_sceneflow(model, loader):
    ...
    for batch_id, data in tqdm(enumerate(loader), total=len(loader), smoothing=0.9):
        pos1, pos2, _, _, flow, path, valid_list = data
        ...
        if torch.sum(valid_list) == valid_list.shape[0]:
            with torch.no_grad():
                pred_flows = model(pos1, pos2, None, None)
                epe_3d, acc_3d, acc_3d_2, _ = evaluate_3d(pred_flows.permute(0, 2, 1).detach().cpu().numpy(),
