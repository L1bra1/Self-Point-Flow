...
from pseudo_labels_utils.Label_generation import KITTI_label_generation
...
def main():
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.7, last_epoch = init_epoch - 1)
...
def train_sceneflow(model, train_loader, optimizer, epoch):
    ...
    for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), smoothing=0.9):
        pos1, pos2, norm1, norm2, _, _, color1, color2 = data
        pos1 = pos1.cuda()
        pos2 = pos2.cuda()
        norm1 = norm1.cuda()
        norm2 = norm2.cuda()
        ...
        with torch.no_grad():
            pseudo_gt = KITTI_label_generation(pred_flows.detach().clone(), pos1, pos2, norm1, norm2, color1, color2)
            pseudo_gt = pseudo_gt.detach()

        # Computing loss
        diff_flow = pred_flows.permute(0, 2, 1) - pseudo_gt.permute(0, 2, 1)
        loss = torch.mean(torch.norm(diff_flow, p=2, dim=2))
